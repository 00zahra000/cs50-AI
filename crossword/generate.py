...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for each in self.domains:
            for word in set(self.domains[each]):
    ...
    def revise(self, x, y):
        ...
        """
        revised = False
        # in which places the overlap happens
        ...
        for x_word in set(self.domains[x]):
            ...
            for y_word in self.domains[y]:
                # if those words have the same letter in those overlapped cells, they can be considered as an option.
        ...
        while arcs:
            ...
            if self.revise(x, y):
                if not self.domains[x]:
                    return False
                # arcs that don't overlap with y ( they're between x and its other neighbors)
    ...
    def consistent(self, assignment):
        ...
        for word in assignment:
            ...
            if assignment[word] not in used_words:
                used_words.add(assignment[word])
            else:
                return False
            ...
            for neighbor in self.crossword.neighbors(word):
                if neighbor in assignment:
                    i, j = self.crossword.overlaps[word, neighbor]
                    if assignment[word][i] != assignment[neighbor][j]:
                        return False
